...
__declspec(dllexport) PacketDecoder::PacketDecoder()
{
...
__declspec(dllexport) void PacketDecoder::SetMaxNumberOfFrames(unsigned int max_num_of_frames)
{
...
__declspec(dllexport) void PacketDecoder::DecodePacket(std::string* data, unsigned int* data_length)
{
...
__declspec(dllexport) void PacketDecoder::SplitFrame()
{
...
__declspec(dllexport) void PacketDecoder::PushFiringData(unsigned char laserId, unsigned short azimuth, unsigned int timestamp, HDLLaserReturn laserReturn, HDLLaserCorrection correction)
{
...
__declspec(dllexport) void PacketDecoder::SetCorrectionsFile(const std::string& corrections_file)
{
...
__declspec(dllexport) void PacketDecoder::UnloadData()
{
  _last_azimuth = 0;
  _frame = new HDLFrame();
  _frames.clear();
...
__declspec(dllexport) void PacketDecoder::InitTables()
{
...
__declspec(dllexport) void PacketDecoder::LoadCorrectionsFile(const std::string& correctionsFile)
{
...
__declspec(dllexport) void PacketDecoder::LoadHDL32Corrections()
{
...
__declspec(dllexport) void PacketDecoder::SetCorrectionsCommon()
{
...
__declspec(dllexport) std::deque<PacketDecoder::HDLFrame> PacketDecoder::GetFrames()
{
  return _frames;
}

__declspec(dllexport) void PacketDecoder::ClearFrames()
{
  _frames.clear();
}

__declspec(dllexport) bool PacketDecoder::GetLatestFrame(PacketDecoder::HDLFrame* frame)
{
  if (_frames.size()) {
    *frame = _frames.back();
    _frames.clear();
