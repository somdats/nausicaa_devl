...
__declspec(dllexport)PacketBundleDecoder::PacketBundleDecoder()
{
...
__declspec(dllexport) void PacketBundleDecoder::SetMaxNumberOfFrames(unsigned int max_num_of_frames)
{
...
__declspec(dllexport)void PacketBundleDecoder::DecodeBundle(std::string* bundle, unsigned int* bundle_length)
{
...
__declspec(dllexport) void PacketBundleDecoder::ProcessHDLPacket(unsigned char *data, unsigned int data_length)
{
  if (data_length != 1206) {
...
__declspec(dllexport) void PacketBundleDecoder::PushFiringData(unsigned char laserId, unsigned short azimuth, unsigned int timestamp, HDLLaserReturn laserReturn, HDLLaserCorrection correction)
{
...
__declspec(dllexport) void PacketBundleDecoder::SetCorrectionsFile(const std::string& corrections_file)
{
...
__declspec(dllexport) void PacketBundleDecoder::UnloadData()
{
  _frame = new HDLFrame();
  _frames.clear();
}

__declspec(dllexport) void PacketBundleDecoder::InitTables()
{
...
__declspec(dllexport) void PacketBundleDecoder::LoadCorrectionsFile(const std::string& correctionsFile)
{
...
__declspec(dllexport) void PacketBundleDecoder::LoadHDL32Corrections()
{
...
__declspec(dllexport) void PacketBundleDecoder::SetCorrectionsCommon()
{
...
__declspec(dllexport) std::deque<PacketBundleDecoder::HDLFrame> PacketBundleDecoder::GetFrames()
{
  return _frames;
}

__declspec(dllexport) void PacketBundleDecoder::ClearFrames()
{
  _frames.clear();
}

__declspec(dllexport) bool PacketBundleDecoder::GetLatestFrame(PacketBundleDecoder::HDLFrame* frame)
{
  if (_frames.size()) {
    *frame = _frames.back();
    _frames.clear();
