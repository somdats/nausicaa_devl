...
public:
	...
	void ContourPtsToCVPts(const std::vector<cv::Point>& cnts, vector<cv::Point2f>& pts);
	void CannyThreshold(const cv::Mat& srcImage);
	std::vector<std::pair<pt2f, pt2f>> detectContours(const cv::Mat& inImage, int lt = 1000, int ht = 1200, cv::Scalar linecolor = cv::Scalar(0, 255, 255));

	vector<cv::Point> getContours(int idx);
