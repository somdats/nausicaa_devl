...
Eigen::Vector2d calib_converter:: WorldToPlane(const Eigen::Vector3d& P, const std::vector<double>& inv_poly,
...
double calib_converter::convertOcam2Mei(const std::vector<double>& poly_inv,
   ...
   //
    ...
    for (int i = 0; i < num_table_count; ++i)
    {
        ...
        for (auto& o : obj_pts)
        {
            ...
            if (transformed.z() > 0)
            {
                auto ip = WorldToPlane(transformed, poly_inv, principal_point,c,d,e);
                if (ip.x() > 30 &&
                    ip.y() > 30 &&
                    ip.x() < img_size.x() - 30 &&
                    ip.y() < img_size.y() - 30)
