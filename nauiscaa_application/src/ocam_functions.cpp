 ...
 void create_perspecive_undistortion_LUT(cv::Mat& mapx, cv::Mat& mapy, struct ocam_model* ocam_model, float sf)
{
     ...
     for (i=0; i<height; i++)
         ...
         {   
            ...
            /* *(mapx.data + i*width+j ) = (float) m[1];
             *( mapy.data + i*width+j ) = (float) m[0];*/
...
//------------------------------------------------------------------------------
//void create_panoramic_undistortion_LUT (cv::Mat* mapx, cv::Mat* mapy, struct ocam_model* ocam_model, float sf)
//{
//     int i, j;
//     float theta;
...
//}
 ...
 void undistortImageOcam(const ocam_model& cd,
     const uint8_t* img,
     int num_ch,
